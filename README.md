...
v1.2.0 进一步增加了时间承诺与日历同步能力：

- `Weekly Time Commitments`：把周计划中的保护时间块、缓冲建议和日历同步对象写成结构化状态
- `Calendar Sync`：预览或执行“周时间承诺 -> 飞书日历”的同步链路
...
- `LMI_FEISHU_IDENTITY`：飞书身份类型，默认 `user`
- `LMI_FEISHU_CALENDAR_ID`：可选，指定写入的日历 ID；不设时优先写入主日历
- `LMI_CALENDAR_TIMEZONE`：时间承诺同步时使用的时区，默认 `Asia/Shanghai`
...
- [LMI_SKILL_USER_GUIDE.md](LMI_SKILL_USER_GUIDE.md)：第一次试用时的中文上手说明
- [AGENTS.md](AGENTS.md)：维护该项目时给 Agent 的工程规则
- [CHANGELOG.md](CHANGELOG.md)：版本更新记录
- [RELEASE_NOTES_v1.2.0.md](RELEASE_NOTES_v1.2.0.md)：最新发布说明
